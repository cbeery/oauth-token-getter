...
DROPBOX_CLIENT_ID = ENV['DROPBOX_CB_AFFY_CLIENT_ID']
DROPBOX_CLIENT_SECRET = ENV['DROPBOX_CB_AFFY_CLIENT_SECRET']
...
get '/dropbox_cb_affy/authorize' do
	url = "https://www.dropbox.com/oauth2/authorize?client_id=#{DROPBOX_CLIENT_ID}&response_type=code&redirect_uri=#{url('/dropbox_cb_affy/callback')}&force_reapprove=false"
	redirect to(url)
end

get '/dropbox_cb_affy/callback' do
	post_params = {client_id: DROPBOX_CLIENT_ID, client_secret: DROPBOX_CLIENT_SECRET, code: params[:code], grant_type: 'authorization_code', redirect_uri: url('/dropbox_cb_affy/callback')}
